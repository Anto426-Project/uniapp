...
fun ExamsScreen(backdropState: Backdrop) {
    ...
    val tabs = listOf(
        LiquidNavigationItem(stringResource(Res.string.ui_bookable_exams), badge = exams.count { !it.isBooked }.toString(), icon = LiquidIcons.Calendar),
        LiquidNavigationItem(stringResource(Res.string.ui_booked_exams), badge = exams.count { it.isBooked }.toString(), icon = LiquidIcons.Check)
    )
